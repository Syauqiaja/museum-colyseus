...
- **Client → server:**
  ...
  - `activity` — `{ game }`: `""` (back in the hall) or one of `MUSEUM_ACTIVITIES`. Only from a listed visitor (after a first `move`). The client sends it as it goes through a doorway and again on coming back; after a dropped socket it re-sends `move` then `activity`.
  - `interact` — `{ station, index }`: an exhibit the visitor set off. `station` is a key of `MUSEUM_STATIONS` (`gong`, `gasing`, `tembang`, `engklek`) and `index` an integer below its count (the petak, 0–7, for `engklek`; 0 otherwise). Only from a listed visitor who is in the hall (`activity === ""`), at most 8 per second (`INTERACTS_PER_SECOND`). Video screens and lesson plaques are deliberately not stations: sharing them would let one visitor start, stop or page them under another.
- **Server → client:**
  - State patches.
  - `interacted` — `{ sessionId, station, index }`: another visitor's accepted `interact`, to everyone except its sender (who has already played it). An event, not state: a visitor who arrives later simply missed it. The Unity client plays it at the station's 3D source, so it is heard only nearby; per-visitor parts (the engklek run, the tembang lyric) are not touched.
  - `error` — `{ code, message }`: `invalid_move` for a malformed `move`; `invalid_activity` for an unknown game or an unlisted sender; `invalid_interact` for an unknown station, an out-of-range index, or a sender not listed or away; `too_fast` past the interaction rate.
